...
#x<-which(small$CellType2%in%c("IL11.CAF","Myocyte","Fibroblast","Endocrine","HSP.Mac","MT1X.Mac","Prolif.Lymph","Prolif.Mac","Progenitor.Lymph"))
#small<-small[,-x]
Idents(small)<-"CellType2"
#==================================================================
table(small$CellType2)
DefaultAssay(small)<-"RNA"
pdacfullmarkers <- FindAllMarkers(small,min.pct = 0.2,min.diff.pct = 0.1,test.use = "MAST",only.pos = F)
saveRDS(pdacfullmarkers,"pdacfullmarkers2022.rds")
...
#=========================================================
PDACSubset1$CellType3<-PDACSubset1$CellType2
PDACSubset1$CellType3<-droplevels(PDACSubset1$CellType3)
PDACSubset1$CellType3<-factor(PDACSubset1$CellType3,levels=c(
...
"Metaplastic"))
PDACSubset1$CellType3<-factor(PDACSubset1$CellType3,levels=rev(levels(PDACSubset1$CellType3)))
Idents(PDACSubset1)<-"CellType3"
DotPlot(PDACSubset1,features=unique(c(stromasec,endosec,myeloidsec,lymphosec,episec)),cluster.idents=F,col.min=0.1,col.max=8,dot.min=0.1,scale.by = "size",dot.scale=6,cols=c("ghostwhite","blue"))+theme(axis.text.x = element_text(angle = 90,  hjust = 1))
#=========================================================
...
DotPlot(subset(PDACTraining,Condition%in%"Tumor"),features=unique(c(stromasec,endosec,myeloidsec,lymphosec,episec)),cluster.idents=F,dot.scale=6)+theme(axis.text.x = element_text(angle = 90,  hjust = 1))
```
