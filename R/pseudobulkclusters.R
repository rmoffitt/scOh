...
#' add(1, 1)
#' add(10, 1)
pseudobulkclusters<-function(object){
  library(dplyr)
  library(purrr)
  #tmp<-subset(object,Condition%in%"Tumor")
  ...
  full.split<-SplitObject(tmp,split.by = c("CellType2"))
  
  means<-array(NA)
  nm<-list()
  for (i in 1:length(full.split)){
    data <- as.matrix(GetAssayData(full.split[[i]],slot = "counts"))
    # data <- as.matrix(GetAssayData(full.split[[i]]))
    
    a <- as.data.frame(rowMeans(data, na.rm=TRUE))
